...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    hinge_loss = np.zeros(labels.shape)
    for k in range(0, labels.shape[0]):
        hinge_loss[k] = hinge_loss_single(feature_matrix[k], labels[k], theta, theta_0)
...
def perceptron_single_step_update(
    ...
    completed.
    """
    z = label * (np.matmul(np.transpose(current_theta), feature_vector) + current_theta_0) 
    theta = current_theta;
...
def classify(feature_matrix, theta, theta_0):
    ...
    pred_int = np.matmul(np.transpose(theta), np.transpose(feature_matrix)) + theta_0
    
    predictions = np.where(pred_int > 0 + 0.000001, 1, -1)
...
def bag_of_words(texts):
    ...
    for text in texts:
        word_list = [i for i in extract_words(text) if i not in stop_words]
